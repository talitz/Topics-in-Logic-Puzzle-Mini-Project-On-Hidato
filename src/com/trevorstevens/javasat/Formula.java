...
public class Formula {
     ...
     */
    private void importCNF( CNF cnf) {


        numVariables = cnf.getMyLiterals().size()/2;
        numClauses = cnf.getMyClauses().size();
        clauseList = new Clause[numClauses];
        int size=(int)Math.sqrt(cnf.getMyLiterals().size()/2);
        int[] intBuffer = new int[numVariables*numClauses];
        int start = 0;
        int clause = 0;
        int end = 0;
        int n=0;
    	for(int i=0;i<cnf.getMyClauses().size();i++)
    	{
    		for(int j=0;j<cnf.getMyClauses().get(i).getNumOfLitrals();j++)
    		{
    			 int num=cnf.getMyClauses().get(i).getMyLiterals().get(j).getId_num()+
    	                  (size*(-1+ 	  
    	            		  cnf.getMyClauses().get(i).getMyLiterals().get(j).getId_ci()));
    			 if(cnf.getMyClauses().get(i).getMyLiterals().get(j).getNot())
    				 num=-num;
    			 intBuffer[n]=num;
    					 n++;
    					 end++;
   ...
   public void forwardTrack() {
     ...
     * @return 
     */
    public int[] printSolution(int size) {
        ...
        while (!hashObjectStack.isEmpty()) {
            if (booleanStack.removeFirst()) {
                num=hashObjectStack.removeFirst().getVariableNumber();
            } else {
